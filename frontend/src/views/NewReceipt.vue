...
async function save() {
  ...
  // 1) Validasi nama pasien wajib diisi
  ...
  if (!form.items.length) {
    const msg = 'Minimal 1 item.'
    error.value = msg
    alert(msg)
    return
  }

  // 3) Validasi bayar >= total
  if (Number(form.paid || 0) < Number(computedReceipt.value.total || 0)) {
    const msg = 'Nominal bayar kurang dari total.'
    error.value = msg
    alert(msg)
    return
  }
  ...
  try {
    ...
    const msg = e.userMessage || 'Gagal menyimpan.'
    error.value = msg
    alert(msg)
  } finally {
    saving.value = false
